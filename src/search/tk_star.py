...
def greedy_best_first_search(task, heuristic, k=1, use_relaxed_plan=False):
    """
    ...
    """
    return top_kstar_search(task, heuristic, k, ordered_node_greedy_best_first,
                        use_relaxed_plan)


def weighted_top_kstar_search(task, heuristic, k=1, weight=5, use_relaxed_plan=False):
    """
    ...
    """
    return top_kstar_search(task, heuristic, k, ordered_node_weighted_astar(weight),
                        use_relaxed_plan)


def top_kstar_search(task, heuristic, k=10, make_open_entry=ordered_node_astar,
    ...
    @param heuristic  A heuristic callable which computes the estimated steps
                      from a search node to reach the goal.
    @param k The number of plans to be returned
    ...
    while open:
        ...
        if state_cost[pop_state] == pop_node.g or tk_searching:
            expansions += 1

            if task.goal_reached(pop_state):
                logging.info("Goal %d reached. Start extraction of solution." % (len(solutions)+1))
                ...
                if len(solutions) == k:
                    ...
                    logging.info("Expanding %d additional nodes" % tk_expansions)
                    continue
            ...
            sidetrack_nodes.append((f, h, _tie, pop_node))
            logging.debug("Adding sidetrack node "+str(pop_node.state))
            tk_expansions -= 1
            if tk_expansions == 0:
                logging.info("Resuming search over %d nodes" % len(sidetrack_nodes))
                for f, h, _tie, node in sidetrack_nodes[:]:
                    heapq.heappush(open, (f, h, _tie, node))
                    state_cost[node] = node.g
                    sidetrack_nodes.remove((f, h, _tie, node))
                tk_searching = True
